...
const DEFAULT_DB_PATH = path.resolve(process.cwd(), 'data', 'pet-vet.sqlite');
const DEFAULT_PUZZLES_PATH = process.env.PUZZLES_JSON_PATH
  ? path.resolve(process.env.PUZZLES_JSON_PATH)
  : path.resolve(__dirname, '../../../shared/puzzles/puzzles.json');
const MIGRATIONS_DIR = path.resolve(__dirname, './migrations');

export class TelemetryStore {
  private readonly db: Database.Database;
  private readonly puzzlePack: PuzzlePack;
  ...
  startSession(input: SessionStartInput): { sessionId: string; userId: string; startedAt: number } {
    const now = input.startedAt ?? Date.now();
    const userId = this.ensureUser(input.userId, input.displayName, now);
    const sessionId = crypto.randomUUID();
    this.db
      .prepare(
        `INSERT INTO sessions (id, user_id, started_at, user_agent, locale)
  ...
  startAttempt(input: AttemptStartInput): { attemptId: string; startedAt: number } {
  ...
  completeAttempt(input: AttemptCompletionInput): AnalyticsAttempt {
    const attempt = this.db
      .prepare(
        `SELECT id, session_id, user_id, puzzle_id, started_at, code_snapshot_json
  ...
  insertEventsBatch(batch: EventsBatchInput): { insertedEvents: number; insertedMovements: number } {
    if (!batch.sessionId) {
    ...
    let attemptRow: { id: string; session_id: string | null; user_id: string | null; puzzle_id: number | null } | null = null;
    if (batch.attemptId) {
    ...
    const insertEventStmt = this.db.prepare(
      `INSERT INTO events (id, session_id, user_id, attempt_id, puzzle_id, ts, type, payload_json)
       VALUES (@id, @session_id, @user_id, @attempt_id, @puzzle_id, @ts, @type, @payload_json)`
    );
    const insertMovementStmt = this.db.prepare(
      `INSERT INTO movements (id, attempt_id, ts, entity, from_x, from_y, to_x, to_y, direction, cause, blocked)
       VALUES (@id, @attempt_id, @ts, @entity, @from_x, @from_y, @to_x, @to_y, @direction, @cause, @blocked)`
    );

    const txn = this.db.transaction((events: EventEnvelope[]) => {
      let movementCount = 0;
  ...
  getPuzzleTimeline(puzzleId: number): { puzzle: PuzzleDefinition; attempts: AnalyticsAttempt[] } {
    ...
    const attempts = this.db
      .prepare(
        `SELECT id, session_id, user_id, puzzle_id, started_at, ended_at, result,
    ...
    const analyticsAttempts = attempts.map((attempt: any) => ({
    ...
    return {
      puzzle: {
        id: puzzle.id,
        title: puzzle.title,
        concepts: puzzle.concepts.split(',').map((c: string) => c.trim()).filter(Boolean),
      },
  ...
  getHealthSnapshot(): { status: 'ok'; puzzles: number; version: string } {
    return {
      status: 'ok',
  ...
  private getMovementsForAttempt(attemptId: string): MovementRow[] {
    const rows = this.db
      .prepare(
        `SELECT id, ts, entity, from_x, from_y, to_x, to_y, direction, cause, blocked
         FROM movements
         WHERE attempt_id = ?
         ORDER BY ts ASC`
      )
  ...
  private ensureUser(userId?: string, displayName?: string, now = Date.now()): string {
    if (userId) {
  ...
  private getSessionRow(sessionId: string): { id: string; user_id: string | null } {
  ...
  private getAttemptRow(attemptId: string): { id: string; session_id: string | null; user_id: string | null; puzzle_id: number | null } {
    const row = this.db
  ...
  private assertPuzzleExists(puzzleId: number): void {
  ...
  private seedPuzzles(): void {
    const insert = this.db.prepare(
      `INSERT INTO puzzles (id, title, concepts, version, checksum)
    ...
    for (const puzzle of this.puzzlePack.puzzles) {
      const checksum = crypto.createHash('sha256').update(JSON.stringify(puzzle)).digest('hex');
      insert.run({
        id: puzzle.id,
        title: puzzle.title,
...
function loadPuzzlePack(puzzlesPath: string): PuzzlePack {
  if (!fs.existsSync(puzzlesPath)) {
    throw new Error(`Puzzle data not found at ${puzzlesPath}`);
  }
  const raw = fs.readFileSync(puzzlesPath, 'utf-8');
  ...
  return parsed as PuzzlePack;
}

function coerceMovement(payload: unknown): {
  entity: string;
  from: { x: number; y: number };
  to: { x: number; y: number };
  direction?: string;
