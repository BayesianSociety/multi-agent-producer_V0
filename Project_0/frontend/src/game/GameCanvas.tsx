...
const targetLabels: Record<string, string> = {
  ...
  treadmill: 'Track',
  splint_table: 'Splint',
  celebration_mat: 'Goal'
};

function formatTargetLabel(type: string | undefined, fallbackId: string): string {
  if (!type) return fallbackId.replace(/[_-]/g, ' ');
  return targetLabels[type] ?? type.replace(/[_-]/g, ' ');
}
      ...
      <div className="scene-panel" style={{ background: sceneBackground }}>
        ...
        <div className="scene-legend" aria-label="Scene legend">
          <span className="legend-item">
            <span className="legend-swatch pet">Pet</span>
            Your character
          </span>
          {puzzle.entities.mentor && (
            <span className="legend-item">
              <span className="legend-swatch mentor">Mentor</span>
              Guide
            </span>
          )}
          {puzzle.entities.targets?.length > 0 && (
            <span className="legend-item">
              <span className="legend-swatch target">Goal</span>
              Objective tile
            </span>
          )}
          <span className="legend-item">
            <span className="legend-swatch trail">Trail</span>
            Travel path
          </span>
        </div>
          ...
          <div className="grid-surface" style={{ width: stageWidth, height: stageHeight }}>
            ...
            {puzzle.entities.targets?.map((target) => (
              <div
                ...
                aria-label={`Goal: ${formatTargetLabel(target.type, target.id)}`}
                style={{
                  left: target.position.x * tileSize + tileSize / 2,
                  top: target.position.y * tileSize + tileSize / 2
                }}
              >
                <span>{formatTargetLabel(target.type, target.id)}</span>
              </div>
            ))}
            {puzzle.entities.objects?.map((object) => (
              <div
                ...
                aria-label={`Item: ${object.itemType ?? object.id}`}
                style={{
