...
1. Give each agent an isolated workspace. For editing tasks, use separate git worktrees.
...
2. Restrict each agent’s scope. Every prompt should state:
- owned files
- forbidden files
...
3. Bound concurrency. Do not run all agents at once unless you have a reason. Use a semaphore, which means use a concurrency limiter that only allows N agents to run at the same time. When one finishes, another can start

4. Capture structured outputs. Do not rely only on prose. Require a parseable footer.
